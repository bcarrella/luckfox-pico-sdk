...
import sys, socket, ssl, re, base64, threading, argparse, imaplib, configparser, daemon
...
class IMAP_Proxy:
    ...
    def __init__(self, port=None, host='', certfile=None, key=DEFAULT_KEY, max_client=MAX_CLIENT, verbose=False, ipv6=False, tls_enabled=False):
    ...
    def new_connection(self, ssock):
        Connection(ssock, self.key, self.verbose, self.tls_enabled)
...
class Connection:
    ...
    def __init__(self, socket, key, verbose = False, tls_enabled = False):
        self.verbose = verbose
        self.key = key
    ...
    def connect_server(self, username, password):
        ...
        if self.tls_enabled:
            ...
            self.conn_server.starttls(ssl_context=tls_context)
    ...
    def starttls(self):
        ...
        self.send_to_client(self.tls_success())
    ...
    def tls_success(self):
        """ Success command completing response """
        return self.client_tag + ' OK Begin TLS negotiation'
...
if __name__ == '__main__':
    ...
    with daemon.DaemonContext():
        IMAP_Proxy(port=port, certfile=certfile, key=key, max_client=nclient, ipv6=ipv6, verbose=verbose, tls_enabled=True)